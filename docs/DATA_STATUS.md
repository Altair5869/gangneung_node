...
| `MOCK_SPOTS` 6곳 전체 필드 | 개발 초기 임시 데모 데이터, **프로덕션 폴백 아님 (2026-07-27부로 변경)** | `lib/spots-data.ts` | 원래 개발자가 직접 조사해서 손으로 입력한 임시 데이터. ~~관광공사 API 응답이 아예 없거나(`catch` 분기) 지역 목록이 비었을 때 fallback으로 쓰임~~ **(2026-07-27) `lib/spot-corpus.ts:45`(0건 응답)·`:101`(`catch` 예외) 두 폴백 경로 모두 `MOCK_SPOTS` 대신 `VERIFIED_SPOTS`(24곳, 실측 완료)만 반환하도록 변경함.** `wifi.speedMbps` 하드코딩(100/500/50/200/300)이 실사용자에게 노출될 수 있었던 경로(CLAUDE.md 규칙 2 위반 소지)가 사라졌다. `MOCK_SPOTS`는 `lib/spots-data.ts`의 `getSpotById`, `app/api/spots/[id]/route.ts`(정적 파라미터 생성용)에서만 여전히 참조되며, `wifi.available`/`power.level`/`noise`/`congestion` 필드의 "실측 아닌 지어낸 확정값" 문제는 이번 변경 범위에 포함되지 않음(별도 후속 티켓 대상, 아래 "남은 일" 참고) |
...
- **(신규, 2026-07-27)** `MOCK_SPOTS`의 `wifi.available`(6곳 전부 확정 boolean)/`power.level`/`noise`/`congestion` 필드가 여전히 "타입은 유효하지만 실측 아닌 지어낸 값"이다 — 이번엔 `speedMbps`(유일한 하드 위반, 필드 자체가 존재하면 안 되는 타입 스키마 수준 위반)만 제거했고, 나머지 필드 재검토는 별도 후속 티켓으로 분리됨(pm-analyst 옵션 (b))
- **(신규, 2026-07-27)** `app/api/spots/[id]/route.ts`의 `getSpotById(id)`는 `lib/spot-corpus.ts`의 폴백 정책과 무관하게, id가 `MOCK_SPOTS`의 고정 id(`"1"`~`"6"`)와 일치하면 관광공사 API 상태와 무관하게 항상 `MOCK_SPOTS` 원본을 반환한다 — "드문 장애 대응 경로"가 아니라 "상시 노출 가능 경로"이므로, 위 (b) 후속 티켓 스코프에 이 파일도 포함할지 pm-analyst 확인 필요 (QA 2026-07-27 발견)
