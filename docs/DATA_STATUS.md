...
| `wifi.speedMbps` | 가짜 (하드코딩), **수집 자체를 포기** | 위와 동일 | 전화로 물어봐도 업체 직원이 답을 모름. 블로그 리뷰에도 정확한 숫자가 거의 없음. 항상 `undefined`/`null`, 검증 조건(`lib/ai.ts`)에서도 제외. **(2026-07-27)** `components/spots/WorkEnvScore.tsx`/`SpotsClient.tsx`의 "WiFi 100Mbps 이상" 채점(+10점)·체크리스트 항목도 동일 원칙으로 제거함 — `MOCK_SPOTS` 폴백에 남아있던 `speedMbps: 100~500` 하드코딩 값 때문에 폴백 발동 시에만 이 항목이 "✓"로 노출되던 문제가 있었음. 만점은 100→90으로 내려갔고(다른 항목에 재분배 안 함), UI 만점 표기(`WorkEnvScore.tsx`의 "/ 90점")도 같이 정정했다. `SCORE_OPTIONS`(`"60점+"`/`"80점+"`, `SpotsClient.tsx`) 값 자체는 안 바꿨다 — "80점+"가 새 만점(90점) 기준으로 더 빡빡해진다는 점만 인지하고 넘어가기로 함 |
