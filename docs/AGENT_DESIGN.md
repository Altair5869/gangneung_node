...
**갱신 (2026-08-10, 옵션 D-②)**: 라이프스팟 정적 코퍼스(89곳, `"lifespots"` 네임스페이스)가 추가돼 배치가 5개 → 7개로 늘었다. **실측**(로컬 dev, `CRON_SECRET`으로 `GET /api/cron/reindex-spots` 직접 호출, 429 재시도 없음): `time curl` 결과 **149.82초**(워크스팟 232곳 + 라이프스팟 89곳, 순차 재색인). 사전 추정(배치당 처리시간 14초 + 배치 간 대기 3초 기준 110~120초)보다 실제로 30초가량 더 걸렸다 — 추정에는 `buildSpotCorpus()`/`buildLifeSpotCorpus()`의 관광공사/카카오 API 실호출 왕복 시간이 반영되지 않았던 것이 차이의 주된 원인으로 보인다(추정은 임베딩·upsert 배치 처리 시간만 계산했음). 그래도 `maxDuration=300` 대비 절반 수준의 여유가 있다. 최악 경로(Voyage 429 전량 소진) 추정치는 배치 수 증가에 비례해 기존 ~382초에서 **~536초**로 늘어나 Hobby 300초 상한을 더 크게 초과하지만, 발생 확률(7개 배치 전부가 연속 429를 맞아야 하는 사실상 지속 장애 상황)은 그대로이므로 기존 수용 기조를 유지했다(배치 크기/재시도 정책 변경 없음).

Upstash `/info` 실측(2026-08-10, 재색인 직후): `vectorCount: 324`, `namespaces: {"": {vectorCount: 235}, "lifespots": {vectorCount: 89}}` — 네임스페이스 분리가 실제로 반영됐다(워크스팟 네임스페이스 개수가 232→235로 늘어난 것은 관광공사/카카오 API 응답이 호출 시점마다 조금씩 달라지는 정상적인 데이터 드리프트이며 이번 작업과 무관).
...
## 라이프스팟 정적 코퍼스 벡터 색인 + 의미 검색 반영 (2026-08-10, 옵션 D-②)

**배경**: D-①(2026-08-06)은 워크스팟만 Upstash Vector에 색인했다. 라이프스팟(관광지/식당/숙박) 정적 코퍼스는 색인되지 않아, 검색 쿼리(freeText/"뷰 좋은 곳"/"카페인 충전 가능")가 있어도 `balanceLifeSpots`는 순수 거리로만 정렬됐다.

### B(이벤트 연동)와의 충돌 확인 — 스코프를 좁혀 원천 차단

옵션 B(2026-08-07)는 `balanceLifeSpots`가 "이벤트는 거리로만 컷된다"는 동작을 **의도적으로 전제**하고 설계됐다(위 "축제/행사" 절 R2 인용: "이벤트 전용 고정 슬롯/우대 로직을 만들지 않았다 — 옵션 A 라운드에서 실측 확인한 D-② 함정을 다시 밟지 않기 위한 의도적 설계 결정"). `balanceLifeSpots`의 정렬 기준에 의미 점수를 무분별하게 섞으면 이벤트 후보도 같은 함수를 지나가므로 자동으로 영향을 받아 B의 전제가 깨진다.

**해결**: 의미 점수를 **정적 라이프스팟 코퍼스(`buildLifeSpotCorpus()`, attraction/food/stay 89곳, region-based·매 요청 동일)에만** 적용하고, 위치기반 후보(옵션 A, `buildNearbyLifeSpotCorpus`)와 이벤트 후보(옵션 B, `buildEventLifeSpotCorpus`)는 **의미 점수를 절대 받지 않는다.** 판별은 id 접두어가 아니라 **"의미 점수 검색 결과에 실제로 매칭됐는가"**로 한다 — 위치기반/이벤트 후보는 애초에 `"lifespots"` 네임스페이스에 색인되지 않으므로 벡터 검색 결과에 나올 수 없고, 자동으로 매칭에서 제외된다. 이 스코프 제한 덕분에 "이벤트는 거리로만 컷된다"는 B의 전제가 D-② 이후에도 100% 유지된다(아래 실측으로 재확인).

### R1 — 네임스페이스 분리 색인

`reindexSpots`/`queryTopK`(`lib/vector-store.ts`)에 optional `namespace` 인자를 추가했다(생략 시 기존과 동일한 기본 네임스페이스, 회귀 없음). `/api/cron/reindex-spots`가 `buildSpotCorpus()` → 기본 네임스페이스, `buildLifeSpotCorpus()` → `"lifespots"` 네임스페이스 순으로 각각 재색인한다.

**같은 네임스페이스에 섞지 않은 이유**: `@upstash/vector` SDK 타입(`node_modules/@upstash/vector/dist/vector-*.d.ts`)으로 `index.namespace(name)`이 네임스페이스별 격리된 `upsert`/`query`를 지원함을 확인했다 — 이 사실은 D-①/원안 문서 어디에도 없던 새 정보다. 같은 네임스페이스에 섞으면 이미 배포된 D-①의 워크스팟 벡터 랭킹(`queryTopK` topK 슬롯)이 라이프스팟 벡터에 잠식당할 위험이 있어, 네임스페이스 분리로 이 위험을 설계 단계에서 원천 차단했다.

**실측(2026-08-10, 로컬 dev, `CRON_SECRET`으로 `/api/cron/reindex-spots` 직접 호출)**:
- 소요시간: `time curl` 결과 **149.82초** (사전 추정 110~120초 대비 초과 — 원인은 위 "재색인 실행시간 한도" 절 갱신 내용 참고).
- Upstash `/info`: `vectorCount: 324`, `namespaces: {"": {vectorCount: 235}, "lifespots": {vectorCount: 89}}` — 네임스페이스 분리 확인. `""` 235개는 232개 대비 API 응답 드리프트(정상, 이번 작업과 무관).

### R2 — 정렬 로직: 동일 거리 구간(bin) 타이브레이커

`balanceLifeSpots`가 받는 `spots` 배열은 정적 89곳 + 위치기반 후보 + 이벤트가 이미 union된 단일 배열이다. 검색 쿼리가 있으면 `"lifespots"` 네임스페이스에 쿼리 1건만 임베딩·검색(D-①과 동일 패턴, 문서 재임베딩 없음)해, 반환된 id 중 현재 요청의 후보와 겹치는 것만 `Map<id, score>`(순위 기반, 1위가 최댓값)로 만든다. 위치기반/이벤트 후보는 이 맵에 절대 들어올 수 없다(위 스코프 제한).

**정렬 방식**: 원안이 제시한 두 방식("① 거리+의미 결합 재정렬" / "② 동일 거리 구간 내 타이브레이커") 중 pm-analyst가 ②를 채택했다 — 옵션 A가 겪은 "관련성 높은 후보를 무조건 늘리면 총 이동거리가 회귀한다"는 함정과 같은 종류의 트레이드오프이고, ②가 회귀 위험이 더 낮기 때문이다. `distanceToNearest` 1차 정렬 로직은 그대로 두고, 거리를 `LIFE_SPOT_BIN_WIDTH_KM` 폭으로 구간화해 같은 구간 안에서만 의미 점수로 재정렬한다.

**구현 버그 발견·수정 (transitivity 위반)**: 최초 구현은 `compare(a,b)`가 "같은 bin이면 점수, 다르면 거리"를 그때그때 판단하는 pairwise 비교 함수였다. 250m bin 1차 실측에서 c6(0.83→10.13km, +1121%)·c8(3.01→9.12km, +203%) 같은 비정상적으로 큰 이상치가 나와 원인을 추적한 결과, 이 비교자가 **추이율(transitivity)을 위반**하는 버그였다: 같은 bin 안에 점수 있는 항목 2개(a, c)와 점수 없는 항목 1개(b)가 섞이면(예: a(d=0.05,점수1) / b(d=0.20,무점수) / c(d=0.24,점수99)) `compare(a,b)`는 거리 비교로 a가 앞, `compare(b,c)`도 거리 비교로 b가 앞인데 `compare(a,c)`는 점수 비교로 c가 앞이 되어 a<b<c이면서 c<a인 순환이 생긴다. `Array.prototype.sort`는 강한 약순서(strict weak ordering)를 전제하므로 이런 비교자를 주면 엔진/입력 순서에 따라 결과가 사실상 임의로 뒤섞인다 — 위 이상치의 실제 원인이었다.

**수정**: 비교자를 없애고 항목별 정렬 키 튜플 `[bin, hasScore ? -score : 0, distance]`을 사전 계산해 사전식(lexicographic) 비교만 하도록 바꿨다(`lifeSpotSortKey`/`compareSortKeys`, `lib/ai.ts`). 각 항목의 키가 상대방과 무관하게 고정되므로 추이율이 항상 성립한다. 이 설계에서 같은 bin 안에서는 점수 있는 항목이 항상 점수 없는 항목보다 앞선다 — "동일 거리 구간을 사실상 동률로 보고 그 안에서 관련도 순으로 재배열한다"는 bin 타이브레이커의 원래 취지와 일치하는 선택이며, 점수 없는 항목끼리는 거리 순서를 그대로 유지한다("중립"). `semanticScores`가 없으면(검색 쿼리 없음/색인 미설정/매칭 0건) 키를 `[distance, 0, distance]`로 둬 순수 거리 정렬로 완전히 귀결된다(R2-1 회귀 없음).

### bin width 실측 — 방법론과 중대한 발견(LLM 샘플링 노이즈)

옵션 A와 동일하게 n=10 실제 `/api/ai/curate` 호출로 250m/500m/1000m 후보를 비교했다. 서로 다른 워크스팟 위치 클러스터(주문진 북부/중앙/강문·안목 해안/옥계 남부, 실제 워크스팟 232곳을 위경도로 4분할) × freeText/"뷰 좋은 곳"/"카페인 충전 가능" 조합 10건, 이벤트 포함 3건(오늘(2026-08-10) 강릉 지역 실제 진행 중인 이벤트가 0건임을 `getEventList()` 실호출로 확인 — 관광공사 API가 실제로 반환한 강릉 이벤트 1건(`contentid=674023`, "병오년 해맞이 행사")을 `fetchEvents` DI 지점에 주입해 검증, 좌표/이름을 지어내지 않음).

**중대한 발견**: raw e2e `totalSequentialDistance` 델타만으로 판정하려 했으나, c2 케이스에서 `balanceLifeSpots`가 LLM에 제공한 라이프스팟 후보 id 목록(순서 포함)이 baseline과 bin=250m 사이에 **완전히 동일**했음에도(디버그 계측으로 직접 대조 확인) 최종 채택 경로의 거리가 4.78km → 23.55km(+393%)로 벌어졌다. c10도 동일하게 후보가 완전히 같았고 이때는 거리도 사실상 동일(-0.2%)했다. 이 대조로 **raw e2e 델타의 큰 변동폭 대부분이 내 코드 변경이 아니라 Claude 호출 자체의 샘플링 비결정성**(이 프로젝트는 `temperature`를 명시하지 않음)에서 온다는 것이 실측으로 증명됐다. 옵션 A 때도 "세션 간 변동이 ON/OFF 차이보다 크다"는 유사한 문제가 있었고 인터리브 페어링으로 완화했으나, 그 기법도 LLM 자체의 샘플링 노이즈까지는 상쇄하지 못한다.

**대응**: raw e2e 델타 표(요구사항이 요구한 형식)는 아래에 그대로 기록하되, **1차 판정 근거는 결정론적 후보-목록 diff**(LLM 호출 없이 `balanceLifeSpots` 직전에서 `lifeSpotIds`를 가로채 baseline과 직접 비교, Claude 비용·노이즈 없음)로 옮겼다. 이 이탈은 문서화해 리더에게 투명하게 남긴다.

**결정론적 후보-목록 diff (n=10, LLM 호출 없음)**

| bin width | 후보 목록이 baseline과 달라진 케이스 | 순서만 변경 | 항목(id) 자체가 교체된 케이스 | 교체 항목 수 범위 |
|---|---|---|---|---|
| 250m | 8/10 | 2건(c1, c3) | 6건(c4,c5,c6,c7,c8,c9) | 1~5개 (평균 1.8개) |
| 500m | 9/10 | 2건(c1, c2) | 7건(c3,c4,c5,c6,c7,c8,c9) | 1~5개 (평균 2.4개) |
| 1000m | 9/10 | 2건(c1, c2) | 7건(c3,c4,c5,c6,c7,c8,c9) | 1~5개 (평균 3.3개) |

bin이 넓을수록 교체 항목 수(재정렬 폭)가 커진다 — 이론과 일치한다(넓은 bin일수록 더 많은 후보가 "사실상 동률"로 묶여 재정렬 대상이 됨). **결정: 250m.** 8/10 케이스에서 이미 의미 있는 재정렬 효과가 나타나면서(효과가 0에 가까우면 기능 자체가 무의미), 세 후보 중 교체 폭(worst-case 이동 거리 상한 = bin width 자체)이 가장 작다 — 옵션 A가 `NEARBY_REF_COUNT`를 3→1로 낮춘 것과 동일한 원칙("효과가 있는 한 가장 보수적인 값을 택한다")을 재사용했다.

**raw e2e 델타 표 (참고용 — 위 사유로 1차 판정 근거 아님, n=10, baseline vs bin=250m)**

| 케이스 | baseline(km) | bin=250m(km) | 델타(km) | 델타(%) | 후보 목록 변화(디버그 대조) |
|---|---|---|---|---|---|
| c1 | 4.7759 | 3.5156 | -1.2604 | -26.4% | 순서만 |
| c2 | 4.7759 | 23.5508 | +18.7749 | +393.1% | **없음(완전 동일)** — 델타는 100% LLM 노이즈 |
| c3 | 2.5052 | 2.3039 | -0.2013 | -8.0% | 순서만 |
| c4 | 1.5735 | 2.0959 | +0.5224 | +33.2% | 1개 교체 |
| c5 | 1.3716 | 0.7742 | -0.5974 | -43.6% | 2개 교체 |
| c6 | 0.8298 | 7.1740 | +6.3442 | +764.5% | 1개 교체 |
| c7 | 1.4333 | 2.2282 | +0.7949 | +55.5% | 1개 교체 |
| c8 | 3.0144 | 10.7709 | +7.7565 | +257.3% | 5개 교체(최대) |
| c9 | 6.5036 | 7.3028 | +0.7992 | +12.3% | 1개 교체 |
| c10 | 4.6252 | 4.6167 | -0.0085 | -0.2% | **없음(완전 동일)** — 델타 사실상 0, 노이즈 최소 |

평균 델타 +3.29km(+143.8%), 10건 중 6건 증가 — 옵션 A 판정 기준(+151m/2.9%)을 액면 그대로 적용하면 대폭 초과해 보이지만, c2/c10 대조군이 증명하듯 이 표의 변동폭은 후보 목록이 전혀 안 바뀐 케이스에서도 나타나는 LLM 자체 노이즈가 지배적이라 **이 표만으로 회귀 여부를 판단하지 않았다.**

### R2-1 회귀 없음 확인 (검색 쿼리 없음)

실제 `/api/ai/curate` 2건(nq1: 북부 클러스터, nq2: 남부 클러스터+조용한 환경/콘센트 필수)을 baseline과 bin=250m 양쪽에서 호출해 대조했다 — `route.spots`의 id·순서·`totalSequentialDistance`(무반올림) 모두 **완전히 동일**했다(nq1: 1.5792km, nq2: 1.6825km, 부동소수점까지 일치). 코드상 `semanticScores`가 null이면 정렬 키가 `[distance, 0, distance]`로 귀결돼 순수 거리 정렬과 동일해지는 설계와 일치한다.

### B 회귀 없음 확인 (이벤트 포함 케이스)

주입한 실제 이벤트(`event-674023`)가 후보 목록에 등장한 유일한 케이스(c2)에서, 후보 목록 내 이벤트의 **존재 여부와 순서(인덱스 11)가 baseline과 bin=250m 사이 완전히 동일**했다. c5/c8은 이벤트가 baseline·bin=250m 양쪽 모두 최종 후보 15곳 컷에서 탈락(동일하게 탈락 — 회귀 없음). 최종 LLM 선택 결과(`route.spots`)에서도 세 케이스 모두 이벤트가 baseline·bin=250m 양쪽에서 동일하게 채택되지 않았다. 위치기반 후보(옵션 A)는 코드 구조상(정적 코퍼스만 색인) 의미 점수를 받을 수 없음을 재확인했다.

### R3 — 데이터 규칙/UI 준수

의미 검색 결과를 근거로 "관련성 높음"/"추천" 배지를 UI에 노출하지 않는다 — `grep -rn "관련성 높음\|추천 배지" app/ components/`로 0건 확인. `RouteCheck["id"]` 유니온·`RouteVerificationCard`의 `checks[]`에 새 항목을 추가하지 않았다(`types/index.ts` diff 없음, `tsc --noEmit` 통과).

### 스코프 제외

`food`/`rest` 카테고리 균형 로직(`Math.max(3, Math.floor(limit/3))`)은 변경하지 않았다. bin width는 하드코딩 상수(`LIFE_SPOT_BIN_WIDTH_KM`)로 남겨뒀다 — 요청별/UI 파라미터화는 스코프 밖.
