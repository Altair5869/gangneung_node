...
// 조기 504가 날 수 있다.
//
// 재실측(2026-08-10, 로컬 dev, 실제 관광공사/Kakao/Voyage/Claude/Upstash 호출, 코퍼스 232곳 기준
// — 옵션 E-2/R6 착수 전 정정. 2026-07-28 실측(12~14초) 이후 위치기반 라이프스팟 조회
// (39531d1), 이벤트 조회(7866f35), 라이프스팟용 queryTopK(0e8c28f) 등 4개 커밋이 이 경로에
// 신규 외부 호출을 추가해 기존 주석이 스테일해진 상태였음):
//   - freeText 없음(혼잡도 정렬만, 벡터/임베딩 미사용): 4.4~6.0초 (4회 측정)
//   - freeText 있음(semanticSort, Upstash Vector 색인 사용): 정상 시 5.8~6.0초, 단 **연속 요청
//     시 Voyage 무료 티어 분당 한도(429)에 실제로 걸림** — 같은 1분 내 2회 연속 호출에서 46.5초,
//     26.8초가 각각 관측됨(embed()의 MAX_RETRIES=3회 × Retry-After 대기가 실제로 소진된 사례,
//     lib/embeddings.ts). 시간을 두고 재요청하면 다시 5.8초대로 돌아옴 — 아래 "최악 추정"이
//     이론상 상한이 아니라 실사용 부하(같은 분에 여러 사용자가 freeText로 조회)에서 실제로
//     재현 가능한 경로임을 이번 실측으로 확인했다.
//   → 위치기반 라이프스팟/이벤트 조회(관광공사, Promise.allSettled로 병렬)가 새로 추가됐지만
//     정상 케이스 총 지연시간은 07-28 대비 오히려 낮게 측정됨(12~14초 → 4.4~6.0초) — 이전
//     실측이 콜드 스타트 등 다른 조건을 포함했을 가능성이 있어 절대값 비교보다는, 신규 커밋들이
//     정상 경로에 수 초 단위 이상의 병목을 추가하지 않았다는 확인 목적으로 사용한다.
// 최악 추정(코드 추적 + 위 429 실측 반영, docs/AGENT_DESIGN.md "Node1/Node2 재시도 루프" 참고):
//   - Voyage 429가 겹치면 embed()가 MAX_RETRIES=3회 × 기본 20초 대기를 전부 소진해 semanticSort
//     한 번이 최대 ~60초까지 늘어날 수 있음(lib/embeddings.ts) — 이번 실측(최대 46.5초)으로
//     이 경로가 실사용 부하에서 실제로 발생함을 확인.
//   - validateRoute가 계속 불합격이면 curationGraph가 generateNode(Claude 호출)를 MAX_ATTEMPTS=3회
//     반복(lib/ai.ts) — 정상 케이스 실측(4.4~6.0초)을 왕복 기준으로 잡아도 3회 합 최대 ~18초.
//   - 랭킹 폴백(최대 ~60초, 요청당 1회만 발생)과 Claude 재시도(최대 ~18초)가 겹치는 최악의 경우
//     합산 ~80초. Vercel Hobby 함수 실행시간 상한(300초, fluid compute 기준, 재색인 라우트 주석
//     참고)에는 여전히 여유가 있으나, 정상 케이스(4.4~6.0초)의 13~18배에 해당하는 값이라 여유를
//     두고 120초로 유지한다 — 사용자 대면 경로라 값을 낮게 잡아 정상 케이스까지 조기 타임아웃
//     나는 쪽이 더 나쁘다고 판단.
